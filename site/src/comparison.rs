...
pub async fn handle_compare(
    ...
    let compile_comparisons = comparison
        ...
        .map(|comparison| {
            ...
            let (datum_a, datum_b) = comparison.comparison.results;
            let percent = if datum_a == 0. {
                if datum_b == 0. {
                    0.
                } else {
                    100.
                ...
                100. * ((datum_b - datum_a) / datum_a)
            };
            api::comparison::CompileTestCaseComparison {
                test_case: api::comparison::CompileTestCase {
        ...
        .sorted_by(|a, b| a.test_case.benchmark.cmp(&b.test_case.benchmark))
        .sorted_by(|a, b| b.percent.abs().partial_cmp(&a.percent.abs()).unwrap())
        .collect();
